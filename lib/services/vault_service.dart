...
class VaultService {
  ...
  Future<PasswordCard> createCard(PasswordPayload payload) async {
    ...
    // Save to database
    await _database.saveCard(card);
    await _database.eventStore.appendEvent(event);
  ...
  Future<PasswordCard?> updateCard(
    ...
    // Save to database
    await _database.saveCard(updatedCard);
    await _database.eventStore.appendEvent(event);
  ...
  Future<bool> deleteCard(String cardId) async {
    ...
    // Apply tombstone
    await _database.deleteCard(cardId, _deviceId!, event.eventId);
    await _database.eventStore.appendEvent(event);
  ...
  Future<PasswordPayload?> decryptCard(PasswordCard card) async {
    ...
    try {
      final encryptedData = EncryptedData(
        ...
        iv: Uint8List(12), // IV is prepended to ciphertext in our implementation
        authTag: Uint8List(16), // Auth tag is appended
      );
...
class Uuid {
  ...
  String v4() {
    ...
    return '$random-${random.substring(0, 4)}-4${random.substring(4, 8)}-a${random.substring(8, 12)}-$random';
  }
}
