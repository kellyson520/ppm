...
class _VaultScreenState extends State<VaultScreen> {
  ...
  void _navigateToAddPassword() {
    ...
    showModalBottomSheet<bool>(
      ...
      builder: (context) => AddPasswordScreen(vaultService: widget.vaultService),
    ).then((result) {
      if (mounted) {
        setState(() {
          _isModalOpen = false;
  ...
  void _navigateToPasswordDetail(PasswordCard card) {
    if (ResponsiveLayout.isExpanded(context)) {
      ...
      showModalBottomSheet(
        ...
        builder: (context) =>
            PasswordDetailScreen(vaultService: widget.vaultService, card: card, isEmbedded: false),
      ).then((_) {
        if (mounted) {
          setState(() {
            _isModalOpen = false;
        ...
        children: [
            ...
            child: Container(
              decoration: const BoxDecoration(
                gradient: RadialGradient(
                  ...
                  radius: 1.5,
                  colors: [Color(0xFF1E1C3A), Color(0xFF101018)],
                  stops: [0.0, 1.0],
                ),
              ),
            ),
          ...
          AnimatedContainer(
            ...
            transform: _isModalOpen
                ? (Matrix4.identity()
                  ..setTranslationRaw(0.0, MediaQuery.of(context).size.height * 0.04, 0.0)
                  ..scale(0.92, 0.92, 1.0))
                : Matrix4.identity(),
            decoration: BoxDecoration(
              borderRadius: _isModalOpen ? BorderRadius.circular(32) : BorderRadius.zero,
            ),
            clipBehavior: _isModalOpen ? Clip.hardEdge : Clip.none,
            child: AnimatedOpacity(
              duration: const Duration(milliseconds: 300),
              opacity: _isModalOpen ? 0.6 : 1.0,
              child: SafeArea(bottom: false, child: _buildBody(l10n)),
            ),
          ),
          Positioned(
            bottom: 30,
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Stack(
          ...
          children: [
            Icon(isActive ? activeIcon : icon, color: isActive ? color : Colors.white54, size: 26),
            Positioned(
              bottom: 6,
              child: AnimatedOpacity(
  ...
  Widget _buildPasswordTab(AppLocalizations l10n) {
    if (ResponsiveLayout.isExpanded(context)) {
      return Row(
        ...
        children: [
          Expanded(flex: 2, child: _buildPasswordListColumn(l10n)),
      ...
      slivers: [
        SliverAppBar(
          ...
          flexibleSpace: FlexibleSpaceBar(
            titlePadding: EdgeInsets.only(left: 24, bottom: 12 + topPadding * 0.3),
            title: Text(
              _getTitle(l10n),
    ...
    return NavigationRail(
      ...
      destinations: [
        NavigationRailDestination(
          ...
          label: Text(
            l10n.passwords,
            style: TextStyle(color: _currentIndex == 0 ? const Color(0xFF6C63FF) : Colors.white54),
          ),
        ),
        NavigationRailDestination(
          icon: const Icon(Icons.verified_user_outlined),
          selectedIcon: const Icon(Icons.verified_user, color: Color(0xFF00BFA6)),
          label: Text(
            l10n.authenticator,
            style: TextStyle(color: _currentIndex == 1 ? const Color(0xFF00BFA6) : Colors.white54),
          ),
        ),
        NavigationRailDestination(
          ...
          label: Text(
            l10n.settings,
            style: TextStyle(color: _currentIndex == 2 ? const Color(0xFFFF6B6B) : Colors.white54),
          ),
        ),
      ],
    );
